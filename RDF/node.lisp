...
(defmethod (setf class-direct-superclasses) :around (new-value
                                                   (instance rdf-node))
  (call-next-method (closette::std-sort-class-list new-value) instance))
...
(defmethod mop:finalize-inheritance :around ((class rdf-node))
  ...
  (if (gethash class *class-table*)
      (format gx-system::*debug-print* "Finalization of ~S delayed~%" class)
      (progn
        (debug-print "~%Finalizing ~S~%Finalization list = ~S~%" class *finalization-list*)
        (if (member class *finalization-list*)
            (format gx-system::*debug-print* "Skipped finalization of ~S because of loop~%" class)
...
(defun compute-direct-slots-list (class)
  ; (format t "Function compute-direct-slots-lists called~%")
...
(defmethod mop:reinitialize-instance :around ((class rdf-node) &rest options)
  (let (classes-to-reinitialize classes-to-finalize new-options)
    (unless (member class *class-architecture*)
      (setf *class-architecture* (closette:std-sort-class-list (push class *class-architecture*))))
    (setf new-options (apply #'foo class options))
;    (setf options (set-difference (apply #'foo class options) options :test #'equal))
    ;(format t "reinitialize-instance (rdf-node)~%    options:~S~%new options:~S~%" options new-options)
    (setf options new-options)
    ;; do we need to do it ?
    ;; (if
    ;;   (every
    ;;   #'(lambda (slot-name)
    ...
    ;;  (progn
       ...
       (format gx-system::*debug-print* "****************~%Reinitialize instance (rdf-node) of ~S with options ~{~%~S~}~%" class options)
       (apply #'call-next-method class 
	      options)
       ;; fix sub/super classes
       #+nil (unless (equal (mop:class-direct-subclasses class)
         ...
         (setf (mop:class-direct-subclasses class)
               (getf options :direct-subclasses)))
       #+nil (unless (equal (mop:class-direct-superclasses class)
       ...
       #+nil (mop:finalize-inheritance class)
       ;(debug-print "About to finalize~S~%" 
		    (mop:class-direct-subclasses class)
	;	    )
       #+nil (loop for c in (mop:class-direct-subclasses class)
...
(defun foo (class &rest options)
  ...
  (let (new-options
       (slot-names (mapcar #'(lambda (s) (slot-value s 'excl::name)) (mop::class-slots class))))
    (setf new-options
          (loop for (option accessor) in *class-options*
             do (funcall accessor class)
	       ;(debug-print "option ~S accessor ~S =>~%result ~S~%" option accessor (funcall accessor class))
             collect option
             collect (cond
                       ;; we add slots
