  ...
  (let ((name (prop-name prop))
    ...
    (let ((resource (getf atts '|rdf|:|resource|))
      ...
      (cond (nodeID
            ...
            (lang
             (cons name (mapcar #'(lambda (val) (cons '@ (list val (intern lang (symbol-name  :keyword)))))
                          (make-form value))))
...
(defun %Description-form (class attrs props)
  ...
  (let ((about (getf attrs '|rdf|:|about|))
    ...
    (when lang
      (when (stringp lang) (setq lang (intern lang (symbol-name :keyword))))
      (remf attrs '|xml|:|lang|))
...
(defun read-rdf-file (accepter-fun &optional (file (ask-user-rdf-file)) (code :default))
  ...
  (with-open-file (stream (pathname file)
                          :external-format
                          #+allegro (excl:find-external-format code)
                          #+lispworks code
                          #+lispworks :element-type
                          #+lispworks 'character #+nil 'simple-char
                          )
    (let ((*line-number* 1)
          (*line-pos* 0)
          (*pos* 0)
