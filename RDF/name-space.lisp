...
(defun ask-user-for-string (prompt string1 option1 option2 prompt2)
  ...
  (progn
    ...
    (let ((str
#+lispworks
            (stream:stream-read-line *standard-input*)
#-lispworks
            (read-line t)
            ))
...
;;;          (t (error "Cant happen!"))))
  ...
  #+nil (format t "~%~S ~S~%~S ~S~%"
          (when ontouri (net.uri:render-uri ontouri nil))
          (when *base-uri* (net.uri:render-uri *base-uri* nil))
          (when ontouri (net.uri:render-uri ontouri nil))
          (when *default-namespace* (net.uri:render-uri *default-namespace* nil))
          )
