     ...
     (typecase object
       (|rdfs|:|Resource| ; type is not rdfs:Class
        ...
        (when
            #+nil
            (handler-case
                (cl:typep type object)
              (conditions:illegal-type-specifier ()
                ; #+lispworks (format t "Illegal type: ~S.~%" object)
		()))
            #+lispworks (and (symbolp object) (getf (symbol-plist object) 'type::type-expansion) (cl:typep type object))
            #+allegro (cl:typep type object)
          (return-from %typep (values nil t)))
