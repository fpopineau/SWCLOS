...
#+lispworks
(defmethod mop::class-direct-superclasses :around ((class |rdfs|:|Class|))
  "Rdfs8 rule is implemented at this method."
...
#+lispworks
(defmethod mop::class-direct-superclasses :around ((class |rdfs|:|Datatype|))
  "Rdfs8 rule is implemented at this method."
  (or (funcall #'call-next-method class)
...
(defun add-direct-slots-to-domain (instance newdomain)
  ...
  (loop for domain in (mklist newdomain)
      ...
      do (let* ((slot-name (name instance))
           ...
           ;; slots
#+(and :lispworks6 (not :lispworks6.1))
           (if (slot-value (class-of instance) 'clos::lock)
               (mp::process-sharing-unlock (slot-value (class-of instance) 'clos::lock)))
...
(defun type-option-check-with-cardinality (instance filler slotd oldval)
  ;(format t "type-option-check-with-cardinality in RdfsKernel ~S ~S ~S ~S~%" instance filler slotd oldval)
  ...
  (let ((supers (getf initargs :direct-superclasses)))
    (when supers             ; direct-superclasses are changed then propagate the effects
      (loop for sub in (mop:class-direct-subclasses class) ; there are some subclasses
         ...
         ; do (format t "~%SharedInitialize:after sub ~S sub-supers ~S~%" sub sub-supers)
