...
(defun addRdfXml (description)
  (addRdfXml-1 description))

(defun addRdfXml-1 (description)
              ...
              (cond ((lang? form) (setq lang-env form))
                    ...
                    (t ;; then form is a name
                     (let ((uri (symbol2uri form))
                           (obj (make-object-with-minimal-contraint form role)))
...
(defun make-object-with-minimal-contraint (name role)
                    ...
                    (cond ((every #'(lambda (abst) (cl:subtypep obj abst)) absts)
                          ...
                          (t ;(format t "~%Adding Class by redefinition2 ~S ~S ~S ~S" classes obj absts islots)
                             (apply #'reinitialize-instance obj
                                    :direct-superclasses (remove-duplicates (append absts (mop::class-direct-superclasses obj)))
                                    initargs))))
                   ...
                   ((rdf-metaclass-p (car classes))   ; class to class
                    ...
                    (cond ((and (null absts) (null initargs))
                          ...
                          ((and (null name) absts)
                           ...
                           (ensure-multiple-classes
                            classes
                            (apply #'mop:ensure-class-using-class obj #+allegro nil #+lispworks (gensym)
                                   :direct-superclasses absts
                                   :metaclass (car classes)
                                   initargs)))
            ...
            ((null name)                                        ; anonymous
             ...
             (set-iri-value
              (ensure-multiple-classes
               classes
               (if absts
                   (apply #'mop:ensure-class-using-class nil #+allegro nil #+lispworks (gensym)
                          :direct-superclasses absts
                          :metaclass (car classes)
                          initargs)
                   (apply #'mop:ensure-class-using-class nil #+allegro nil #+lispworks (gensym)
    ...
    (flet ((ensure-multiple-classes (classes obj)
      ...
      (cond ((and (null slots) (eq (car classes) (class-of obj)))
            ...
            ((and name (null obj) (not (boundp name)))  ; new name and may be ensured to multiple classes.
             ;(format t "~%NEW NAME ~S~%instance ~S initargs~S~%" name (car classes) initargs)
             (when (equal name '|owl|:|intersectionOf|)
