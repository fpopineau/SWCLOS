...
(defmethod excl::compute-effective-slot-definition-initargs ((class |rdfs|:|Class|) #+lispworks name direct-slotds)
  "see above"
  (declare (optimize (speed 3) (safety 0)) #+lispworks (ignore name))
  ; (format t "~%compute-effective-slot-definition-initarg class ~S direct-slotds ~S~%" class direct-slotds)
...
(defmethod mop:effective-slot-definition-class ((class |rdfs|:|Class|) &rest initargs)
  "see above"
  (declare (optimize (speed 3) (safety 0)))
  ;(format t "mop:effective-slot-definition-class class ~S initargs ~S~%" class initargs)
...
(mop::reinitialize-instance (find-class '|rdfs|:|Datatype|)
                            ...
                            `((:name form
                                     ...
                                     :initargs (:form)
                                     :readers (type-form)
                                     :writers ((setf type-form))
                                     ))
                            :metaclass (find-class '|rdfs|:|Class|)
                            )
...
#+lispworks
(mop::reinitialize-instance (find-class '|rdf|:|PlainLiteral|)
                            :direct-slots
                            `((:name value
                                     :initform nil
                                     :initfunction ,(load-time-value #'excl::false)
                            ...
                            :metaclass (find-class '|rdfs|:|Datatype|)
                            )
