...
(defpackage :gx
  (:export |rdfs:Resource| metaRDFSclass rdfsClass *base-uri* *reify-p*
...
(eval-when (:load-toplevel :compile-toplevel :execute)
  (push (find-class '|rdfs|:|Class|) *class-architecture*)
)
...
#+lispworks
(mop::reinitialize-instance (find-class 'rdfsClass))
...
rdfs:Resource."))
#+lispworks
(mop::reinitialize-instance (find-class '|rdfs|:|Resource|))

#+nil
(print-relations 3)

#+lispworks
(defmethod class-direct-superclasses ((class (eql (find-class '|rdfs|:|Resource|))))
  (list (find-class 'gnode)))

#+lispworks
(defmethod class-direct-superclasses ((class (eql (find-class '|rdfs|:|Class|))))
  (list (find-class '|rdfs|:|Resource|) (find-class 'rdf-node)))

#+lispworks
(defmethod validate-superclass ((class |rdfs|:|Class|) (superclass standard-class))
  t)

#+lispworks
(defmethod validate-superclass ((class standard-class) (superclass |rdfs|:|Class|))
  t)

#+lispworks
(defmethod validate-superclass ((class |rdfs|:|Class|) (superclass |rdfs|:|Resource|))
  t)

#+lispworks
(defmethod validate-superclass ((class |rdfs|:|Resource|) (superclass |rdfs|:|Class|))
  t)
...
#+nil
(print-relations '4-pre)

#+lispworks
(mop::reinitialize-instance (find-class '|rdfs|:|Class|)
                           :direct-superclasses (list (find-class '|rdfs|:|Resource|)
                                                      (find-class 'rdf-node))
                           :metaclass (find-class 'rdfsClass)
                           )
#+nil
(print-relations 4)
...
(mop::reinitialize-instance (load-time-value |rdf|:|Property|)
                       :mclasses `(,(load-time-value |rdfs|:|Class|)))

(defparameter |owl|:|ObjectProperty|
...
(defmethod mop:direct-slot-definition-class ((class |rdfs|:|Class|) &rest initargs)
  ...
  (let ((debug-print t))
  ;(format debug-print "direct-slot-definition-class class ~S initargs ~S " class initargs)
  (if (or ; (null (getf initargs :initargs))
       (keywordp (car (getf initargs :initargs))))
      (progn ;(format debug-print "=> standard slot~%")
             (call-next-method))
      (case (getf initargs :name)
	((|rdf|:|about| |rdf|:|ID| |xml|:|lang|) ;(format debug-print " => standard slot~%") 
	 (call-next-method))
	((|rdf|:|type| |rdfs|:|subClassOf| |rdfs|:|label| |rdfs|:|isDefinedBy| |rdfs|:|comment|
	 ...
	 ;(format debug-print " => gx::Property-direct-slot-definition~%")
		...
		(let* ((prop (symbol-value (getf initargs :name)))
		  ...
		  (cond ((and (consp domains)
			...
			((and domains (eq (cg::name domains) '|owl|:|Restriction|))
			 ;(format debug-print " => gx::Property-direct-slot-definition~%") ; FP: Potential problem if debug-print doesn't return t
			 (find-class 'gx::Property-direct-slot-definition))
			((and (find-class '|owl|:|ObjectProperty| nil) (cl:typep prop '|owl|:|ObjectProperty|))
			 ;(format t " => OwlProperty-direct-slot-definition~%")
			 (find-class 'OwlProperty-direct-slot-definition))
			(t ;(format debug-print " => ~S~%" *default-slot-definition-class*)
			   (find-class *default-slot-definition-class*)))))
             (t ;; non-keyword symbol should be a role name.
              (cond ((cl:typep (symbol-value (getf initargs :name)) '|owl|:|ObjectProperty|)
		     ;(format debug-print " => OwlProperty-direct-slot-definition~%")
                     (find-class 'OwlProperty-direct-slot-definition))
                    (t ;(format debug-print " => ~S~%" *default-slot-definition-class*)
                       (find-class *default-slot-definition-class*))))))))))
...
#+lispworks
(defmethod mop::compute-effective-slot-definition-initargs ((class |rdfs|:|Class|) name direct-slotds)
  "see above"
  (declare (optimize (speed 3) (safety 0))
           (ignore name))
  (let ((initargs (call-next-method)))
    ;(format t "RdfBoot: after (call-next-method)~%")
    (cond ((member-if #'property-direct-slotd-p direct-slotds)
           ;; if a slotd is property slotd, add subject-type option.
           `(:subject-type ,class ,@initargs))
          (t initargs))))
...
(mop::reinitialize-instance (load-time-value |rdfs|:|Class|)
                       :direct-slots
...
#+nil
(print-relations '9-pre)

#+lispworks
(setf (mop:class-direct-subclasses |rdfs|:|Resource|)
      (list (find-class '|rdf|:|Property|)
            (find-class '|rdfs:Resource|)
            (find-class '|rdfs|:|Class|)))
#+lispworks
(mop::reinitialize-instance |rdfs|:|Resource|)
...
(defparameter |rdfs|:|Datatype| (find-class '|rdfs|:|Datatype|)
  "rdfs:Datatype is a subclass of and an instance of rdfs:Class.")
#+lispworks
(mop::reinitialize-instance (find-class '|rdfs|:|Datatype|))

#+nil
(print-relations 10)

#+lispworks
(defmethod mop:class-direct-slots ((c (eql |rdfs|:|Datatype|))) () )

#+lispworks
(defmethod mop:class-direct-subclasses ((c (eql |rdfs|:|Datatype|))) () )
...
#+lispworks
(mop::reinitialize-instance (find-class '|rdfs|:|Literal|))

#+nil
(print-relations 12)

(mop::reinitialize-instance (load-time-value |rdfs|:|Literal|)
                       :mclasses `(,(load-time-value |rdfs|:|Class|)))
...
#+lispworks
(mop::reinitialize-instance (find-class '|rdf|:|XMLLiteral|))

#+nil
(print-relations 14)

;; (defmethod mop:class-direct-subclasses ((c (eql |rdf|:|XMLLiteral|))) () )
...
(mop::reinitialize-instance (load-time-value |rdfs|:|Resource|)
                       :direct-slots
                       `(
                       ...
                       :direct-superclasses (list (find-class 'gnode ))
                       :direct-subclasses (mapcar #'find-class
                                                  '(|rdfs|:|Literal| |rdf|:|Property| |rdfs:Resource| |rdfs|:|Class|))
                       :metaclass |rdfs|:|Class|
                       :mclasses `(,(load-time-value |rdfs|:|Class|)))
...
#+nil
(print-relations '17-pre)

#+lispworks
(mop::reinitialize-instance (find-class '|rdf|:|List|))
...
#+nil
(print-relations '19-pre)

;(format t "class-slots rdfs:Resource: ~{~%~S~}~%" (mop:class-slots |rdfs|:|Resource|))

(mop::reinitialize-instance (load-time-value |rdfs|:|Class|)
                       :direct-instances `(,(load-time-value |rdf|:|Property|)
                                           ,(load-time-value |rdfs|:|Resource|)
                                           ,(load-time-value |rdfs|:|Class|))
                       :direct-superclasses (list |rdfs|:|Resource|)
                       :mclasses `(,(load-time-value |rdfs|:|Class|)))
