...
(cl:defpackage :gx
  ...
  (:export *NameSpaces* *default-namespace* *base-uri* set-uri-namedspace
           name uri2symbol line
           *uri2symbol-name-mapping-fun* *uri2symbol-package-mapping-fun* *uri2symbol-user-mapping-fun*
   ...
   (lambda (condition stream)
     (let ((fcont (simple-condition-format-control condition))
       ...
       (format stream "invalid RDF parse: ~A: ~S at line ~S"
         (apply #'format nil fcont args) (expose-buf) *line-number*)))))
                                 ...
                                 (cond ((char= (car (last pat)) q) pat)
                                       (t (setq pat (coerce pat 'cl:string))
                                          (let ((found (cdr (assoc pat *entity-decls* :test #'string=))))
                                            (cond (found)
                                                  (t (error "There is no entity definition for ~A at line ~S." pat *line-number*)
                                                     )))))))))
                            ...
                            (let ((pat (loop for cc = (getnext-char stream)
                              ...
                              (let ((found (cdr (assoc pat *entity-decls* :test #'string=))))
                                (cond (found)
                                      (t (error "There is no entity definition for ~A at line ~S." pat *line-number*))))))
                           ((char= c #\<)
                            (error "Illegal AttValue with '<' at line number ~S" *line-number*))
                           (t c)))))
    (when value
      (setq value
            (loop for c in value
...
(defun read-Attribute-in-Description (stream)
  ...
  (let (Name)
    (cond ((match-pattern-p "xmlns:" stream) ; PrefixedAttName
           (error "'xmlns:' in description at line ~S" *line-number*))
...
(defun read-STag-or-EmptyElemTag-in-Description (stream)
  ...
  (cond ((read-pattern-p "<" stream)
        ...
        (t (error "Illegal STag at line number ~S." *line-number*))))
...
10, an instance of cl:integer, then 10^^|xsd|:|integer| is made, in which the value is 10."
  ...
  (ecase datatype
    ...
    (|xsd|:|duration| (parse-as-duration value))
...
(defun ^^ (value type)
  ...
  (flet ((name (x) (if (symbolp x) x (name x))))
    (typecase value
...
(defun make-form (source)
  (typecase source
    (prop (slots-form (prop-name source) (prop-att&vals source) (prop-value source)))
    (Description (description-form (Description-tag source) (Description-att&vals source) (Description-elements source)))
    (RDF (rdf-form (RDF-att&vals source) (RDF-body source)))
    ))
...
(defun slots-form (name attvals value)
  (cond ((null attvals) `(,name ,@(mapcar #'make-form value)))
        ((eq (car attvals) '|rdf|:|parseType|)
         (cond ((string= (second attvals) "Resource")
                `(,(parse-uri value) ,@(attvals->slots (cddr attvals))))
               ((string= (second attvals) "Collection")
                )
               )
         )
        )
