...
(defvar lang-env nil "xml:lang environment")
...
(excl::without-redefinition-warnings
   ...
   and returns the class object."
      (assert (symbolp name))
      `(progn
         ;; With Lispworks, use the dspec location tracking (dspec:location)
         #-lispworks
...
(defun addRdfXml (description)
  ...
  (cond ((|Description|-p description)
         (let* ((form (|Description|-form description))
           ...
           ;(format t "~%addRdfXML: ~S" (list* (car form) `(:name ,name) (cdr form)))
		     ...
		     ;(format t "~%Form:~S" form)
		       ...
		       ;(format t "~%Domains:~S" domains)
                       ...
                       (let ((class
                             ...
                             (obj nil))
                         ;(format t "~%Class: ~S~%slots: ~S~%obj: ~S" class slots obj)
...
;(trace (gx::most-specific-concepts :inside gx::addForm))
;(trace (gx::collect-domains :inside gx::addForm))
;(trace (gx::%clos-subtype-p :inside gx::addForm))
    ...
    (flet ((ensure-multiple-classes (metas cls)
           ...
           (set-iri-value (obj)
             "binds <obj> to obj's uri of rdf:ID or rdf:about attribute value,
              and push <obj> into *subjects-defined*"
                          #+allegro (with-slots (|rdf|:|about| |rdf|:|ID|) obj
                          ...
                          #+lispworks
                          (cond ((slot-boundp obj '|rdf|:|about|) (setf (iri-value (iri (slot-value obj '|rdf|:|about|))) obj))
                                  ((slot-boundp obj '|rdf|:|ID|) (setf (iri-value (symbol2uri (slot-value obj '|rdf|:|ID|))) obj)))
      ...
      (cond (obj                                                  ; redefining
             ...
             (cond ((every #'(lambda (cls) (cl:typep obj cls)) classes)
                    ...
                    (cond ((every #'(lambda (abst) (cl:subtypep obj abst)) absts)
                          ...
                          (t ;(format t "~%Adding Class by redefinition2 ~S ~S ~S ~S" classes obj absts islots)
                             (apply #'reinitialize-instance obj
                                    :direct-superclasses #+allegro absts #+lispworks (remove-duplicates (append absts (mop::class-direct-superclasses obj)))
                                    initargs))))
                   ((%instance-p obj)                          ; instance to class
                    ...
                    ;(format t "ensure-class-using-class: obj = ~S name = ~S~%" obj name)
                    ...
                    (cond ((and (null absts) (null initargs))
                           (cond ((subtypep (class-of obj) (car classes)) ; only check multiple classing
                                 ...
                                 (t (warn "(~S type ~S) directed but it violates monotonicity, then no effect."
                                          obj (car classes)))))
                          ((and name absts)
                           ;(format t "ensure-class: name = ~S~%" obj name)
                           (ensure-multiple-classes
                            classes
                          ...
                          ((and (null name) absts)
                           ...
                           (ensure-multiple-classes
                            classes
                            (apply #'mop:ensure-class-using-class obj (gensym)
                                   :direct-superclasses absts
                                   :metaclass (car classes)
                                   initargs)))
            ...
            ((and name (null obj))                  ; new name or
             ...
             ;(format t "ensure-class: name = ~S~%" name)
             (set-iri-value
              (ensure-multiple-classes
               classes
               (if absts
            ...
            ((null name)                                        ; anonymous
             ...
             ;(format t "ensure-class-using-class name ~S super ~S classes ~S~%" name absts classes)
             (set-iri-value
              (ensure-multiple-classes
               classes
               (if absts
                   (apply #'mop:ensure-class-using-class nil (gensym)
                          :direct-superclasses absts
                          :metaclass (car classes)
                          initargs)
                   (apply #'mop:ensure-class-using-class nil (gensym)
                          :metaclass (car classes)
                          initargs)))))
            (t  ;(format t "Obj nil, leaving addClass~%")
	     nil)
            ))))
            ...
            append (let ((role (slot-role slot))
                     ...
                     (cond ((null fillers) (list role :unbound))
                           ...
                           (t (warn "Entail by rdfs1: ~S rdf:type rdf:Property." role)
                              ...
                              (cond ((cdr fillers) (list role fillers))
                                    (t slot))))))))
    ;(format t "~%Initargs: ~S~%Name = ~S~%" initargs name)
      ...
      (cond ((and (null slots) (eq (car classes) (class-of obj)))
            ...
            (obj                                                   ; redefine
             ...
             (let ((mclasses (substitute |rdfs|:|Resource| |rdfs:Resource| (mclasses obj))))
               #+nil (format t "~%classes = ~S mclasses = ~S" classes mclasses)
               (cond ((every #'(lambda (cls) (cl:typep obj cls)) classes)
                     ...
                     ((null (cdr classes)) ; different single class, maybe forward ref to regular def
                      (let ((newcls (car classes)))
                        (cond ((eq newcls |rdfs:Resource|)
                              ...
                              (t (let* ((oldcls (class-of obj))
                                        (msc (most-specific-concepts (append mclasses classes)))
                                        (shadow (make-shadow oldcls msc)))
                                   (unless (equal oldcls shadow)
            ...
            ((and name (null obj) (not (boundp name)))  ; new name and may be ensured to multiple classes.
             #+nil (format t "~%NEW NAME ~S~%instance ~S initargs~S~%" name (car classes) initargs)
             #+nil (when (equal name '|owl|:|intersectionOf|)
                     (describe (find-class '|rdf|:|Property|)))
             (set-iri-value
              (ensure-multiple-classes
            ...
            ((null name)                   ; anonymous but may be ensured to multiple classes
             ;(format t "Null name for ~S~%" (car classes))
             (set-iri-value
              (ensure-multiple-classes
