...
(defun langString-p (str i)
  ...
  (and (char= (char str i) #\")
       ...
       (incf i) ; next of #\"
       (cond ((= i (length str)) t)
             ((and (cl::< i (length str)) (char= (char str i) #\@) (language-p str (1+ i))))
             ((and (cl::< i (length str)) (char= (char str i) #\^) (char= (char str (1+ i)) #\^) (uriref-p str (+ i 2)))
              nil)
             (t t))))
(defun parse-langString (str i)
  "langString ::= '\"' string '\"' ( '@' language )?"
  (let ((string (subseq str i (setq i (1+ (position-with-escape #\" str (1+ i))))))) ; #\" to #\"
    (cond ((and (cl::< i (length str)) (char= (char str i) #\@) (language-p str (1+ i)))
           (let ((lang nil))
...
(defun datatypeString-p (str i)
  ...
  (and (char= (char str i) #\")
       (numberp (setq i (position-with-escape #\" str (1+ i))))
       (incf i) ; next of #\"
       (and (cl::< (1+ i) (length str)) (char= (char str i) #\^) (char= (char str (1+ i)) #\^)
            (uriref-p str (+ i 2)))))
