...
(defclass |owl|:|Class| (|rdfs|:|Class|)
  ...
  (:documentation "The meta class in OWL universe. This class is a subclass and instance of rdfs:Class."))
(reinitialize-instance (find-class '|owl|:|Class|))
(print-relations 25)
...
(reinitialize-instance (find-class '|owl|:|Thing|))
(print-relations 27)
...
(reinitialize-instance (find-class '|owl|:|ObjectProperty|))
...
(reinitialize-instance (find-class '|owl|:|Restriction|))
...
#+lispworks
(defmethod mop::class-direct-superclasses :around ((class |owl|:|Restriction|))
  "The default direct superclass of restrictions is rdfs:Resource."
  ...
  (cond ((and (slot-exists-p obj 'excl::name)
        ...
        ((and (slot-exists-p obj '|owl|:|onProperty|)
         ...
         (print-unreadable-object (obj stream :type nil)
           (format stream "Å‡•Ü‡§Ö‡§í~S.~S"
             (name (slot-value obj '|owl|:|onProperty|))
             (or (name (slot-value obj '|owl|:|allValuesFrom|))
                 (and (slot-boundp obj '|owl|:|allValuesFrom|)
                      (get-form (slot-value obj '|owl|:|allValuesFrom|)))))))
        (t (call-next-method))))
(reinitialize-instance (find-class '|owl|:|allValuesFromRestriction|))
...
(reinitialize-instance (find-class '|owl|:|cardinalityRestriction|))
...
; (excl:without-redefinition-warnings
(defmethod mop::compute-effective-slot-definition-initargs ((class |rdfs|:|Class|) #+lispworks name direct-slotds)
  (declare (optimize (speed 3) (safety 0))
           (ignore name))
  (let ((initargs (call-next-method)))
    ;(format t "OwlKernelBoot: after (call-next-method)~%")
...
(defun type-option-check-with-cardinality (instance filler slotd oldval)
  ; (format t "type-option-check-with-cardinality in OwlKernel ~S ~S ~S ~S~%" instance filler slotd oldval)
  ...
  (let ((type (mop:slot-definition-type slotd))
    ...
    ; (format t "type = ~S name = ~S~%" type name)
    (typecase type
...
#+lispworks
(defmethod mop::class-direct-superclasses ((class |owl|:|Class|))
  (or (funcall #'call-next-method class)
      (list (load-time-value (find-class '|owl|:|Thing|)))))
(print-relations 29)
...
;;(trace addRdfXml-1 shared-initialize-before-in-RDF collect-ranges collect-domains read-QName (addClass :inside addObject :step nil)  addForm %addForm addObject)

#+nil
(read-rdf-file #'addRdfXml "OWL/OWL.rdf")

;(format t "OWL.rdf read~%")
;;; ==================================================================================
;;;
;;; Note that
...
#+nil
(defmethod change-class :before ((from gx::rdf-node) (to gx::rdf-node) &rest initargs) ;bug3166
  (declare (ignore initargs))		;bug3166
  (unless (mop:class-finalized-p to)	;bug3254
    (mop:finalize-inheritance to))
  (format nil "~S" (mop:class-prototype to))    ; this is magic code.
  (unless (excl::validate-metaclass-change from (mop:class-prototype to))
    (error "validate-metaclass-change forbids changing the class of ~S to ~S~%" from to))
)
