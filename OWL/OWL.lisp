...
(defmethod shared-initialize :before ((class |owl|:|Class|) slot-names &rest initargs
  ...
  (let ((supers (append (mklist (getf initargs '|rdfs|:|subClassOf|))
                        (mklist (getf initargs '|owl|:|intersectionOf|)))))
    (when supers
    ...
    (when (and (null slot-names) (not (null initargs))) ; reinitialize
      (loop for (role newval) on initargs by #'cddr with oldval
          when (and (or (eq role '|owl|:|intersectionOf|) (eq role '|owl|:|unionOf|))
                    ...
                    ;; not set equal
                    (not (and (subsetp (mklist  oldval) (mklist  newval) :test #'owl-same-p)
                              (subsetp (mklist newval) (mklist oldval) :test #'owl-same-p))))
...
(defmethod update-instance-for-different-class ((previous |rdfs|:|Class|) (current |owl|:|Class|)
  ...
  (cond ((and (not (cl:typep previous |owl|:|Class|))     ; rdfsClass to owl:Class
	 ...
	 ;(format t "~%LWW:update-instance-for-different-class initargs = ~S"
	 ;	 (getf initargs :direct-superclasses))
         (setf (getf initargs :direct-superclasses)
           (cons |owl|:|Thing| (getf initargs :direct-superclasses)))
	 ;(format t " => initargs = ~S~%" (getf initargs :direct-superclasses))
...
(defmethod shared-initialize :after ((class |owl|:|Class|) slot-names &rest initargs)
  (cond ((and (null slot-names) (null initargs))  ; when metaclass changed
        ...
        (t ;; first or redefinition
         #+nil (with-slots (|rdfs|:|subClassOf| |owl|:|intersectionOf| |owl|:|unionOf| |owl|:|equivalentClass|
           ...
           (when (and (slot-boundp class '|rdfs|:|subClassOf|) (slot-value class '|rdfs|:|subClassOf|))
             (check-intersection-refining-for-subclasses class (mklist (slot-value class '|rdfs|:|subClassOf|)))
             (when (mop:class-direct-subclasses class)
               (check-union-refining-for-subclasses class (mop:class-direct-subclasses class))))
           (when (and (slot-boundp class '|owl|:|intersectionOf|) (slot-value class '|owl|:|intersectionOf|))
             (shared-initialize-after-for-intersectionOf class (slot-value class '|owl|:|intersectionOf|)))
           (when (and (slot-boundp class '|owl|:|unionOf|) (slot-value class '|owl|:|unionOf|))
             (unless (eq class |owl|:|Thing|)
               (shared-initialize-after-for-unionOf class (slot-value class '|owl|:|unionOf|))))
           (when (and (slot-boundp class '|owl|:|equivalentClass|) (slot-value class '|owl|:|equivalentClass|))
             (shared-initialize-after-for-equivalentClass class (mklist (slot-value class '|owl|:|equivalentClass|))))
           (when (and (slot-boundp class '|owl|:|disjointWith|) (slot-value class '|owl|:|disjointWith|))
             (shared-initialize-after-for-disjointWith class (mklist (slot-value class '|owl|:|disjointWith|))))
           (when (and (slot-boundp class '|owl|:|complementOf|) (slot-value class '|owl|:|complementOf|))
             (shared-initialize-after-for-complementOf class (mklist (slot-value class '|owl|:|complementOf|))))

         (unless (owl-thing-p class)
	   ;(format t "~%LWW:shared-initialize :after owl:Class direct-superclasses = ~S~%"
	   ; 	   (list |owl|:|Thing| (mop:class-direct-superclasses class)))
...
(defmethod shared-initialize :after ((instance |owl|:|Thing|) slot-names &rest initargs)
  (cond ((and (null slot-names) (null initargs))  ; when class changed
        ...
        (t                                        ; first or redefinition
         ...
         ;; FIXME: doesn't seem to get much from using with-slots ?
         ;; LWW: |owl|:|sameAs| is specially bound -> error
         #+nil (with-slots (|owl|:|sameAs| |owl|:|differentFrom|) instance
         ...
         ;; owl:sameAs makes sameAs groups among individuals.
         (when (slot-boundp instance '|owl|:|sameAs|)
           (shared-initialize-after-for-sameAs instance (mklist (slot-value instance '|owl|:|sameAs|))))
         ;; owl:differentFrom makes pairwise different groups among individuals.
         (when (slot-boundp instance '|owl|:|differentFrom|)
           (shared-initialize-after-for-differentFrom instance (mklist (slot-value instance '|owl|:|differentFrom|))))
...
(defmethod make-this-supers ((class |owl|:|Class|) superclasses)
  ...
  (let ((restrictions (remove-if-not #'owl-restriction-p superclasses))
	...
	;(format t "~%LWW:make-this-supers ~S~%" (cons |owl|:|Thing| supers))
                                                         ...
                                                         (flet ((get-card (obj prop)
                                                           ...
                                                           (let ((maxspec
                                                             ...
                                                             (and (or (null minabst) (null minspec)
                                                                      (cl::> (value-of minspec) (value-of minabst)))
                                                                  (or (null maxabst) (null maxspec)
                                                                      (cl::< (value-of maxspec) (value-of maxabst)))))))))))
                                              t))))))
        (assert answer)
        answer))))
...
(defmethod mop:ensure-class-using-class ((class |owl|:|Class|) name &rest args)
  ...
  (cond ((eq class |owl|:|Thing|) class)                              ; nothing done
        ...
        (t (let ((initargs (copy-list args)))
	     ;(format t "~%LWW:ensure-class-using-class owl:Class~%")
                   ...
                   (t (case name
                        ...
                        (otherwise
                         (mop::reinitialize-instance
                          class
                          :direct-slots
                          `((:documentation ,(format nil "From hasValueRestriction ~S" class)
                             :name ,name :initargs (,name)
                             :type ,(make-instance 'fills
                                                   :role name
                                                   :filler hasvalues
        ...
        (t ;; first or redefinition
         (let ((property (slot-value class '|owl|:|onProperty|)))
           (let* ((name (name property))
             ...
             (cond (slotd
                    ;(format t "~%Reinitializing slotd with cardinality ~S~%" cardinality)
                    #+nil (reinitialize-instance slotd
                                           :name name
                   ...
                   (t
                    ;(format t "Creating cardinalityRestriction~%" slotd)
