...
(eval-when (:load-toplevel :execute)
  (defparameter *owl-directory*
    (merge-pathnames #P"local-projects/swclos2/owl/" ql:*quicklisp-home*))
...
(defmethod mop::compute-effective-slot-definition-initargs :around ((class |rdfs|:|Class|) #+lispworks name direct-slotds)
  (declare (optimize (speed 3) (safety 0))
           #+lispworks (ignore name))
...
(eval-when (:load-toplevel)
  (let ((*default-pathname-defaults* *load-pathname*))
    #+nil (trace shared-initialize-before-in-RDF collect-ranges collect-domains read-QName (addClass :inside addObject :step nil)  addForm %addForm addObject name-ontology change-class)
